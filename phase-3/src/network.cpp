	...
	std::string& Summary, std::string& Director)
{
	...
	if (Name == "" || Year == -1 || Length == 0 || Price == -1 || Director == "")
		throw BadRequest();
}
...
std::vector<Film*> Network::ShowDetailsOfFilm(std::string CommandLine)
{
	int FilmID = 0;
	int index = CommandLine.find(QUESTION_MARK);
	if (CommandLine[index + 1] == ' ')
	...
	return RecommendFilms(FilmID, Graph->GetRecommendation(FilmID));
}
...
std::vector<Film*> Network::RecommendFilms(int FilmID, std::vector<int> ListOfRecommendedFilms)
{
	std::vector<Film*> RecommendationList;
	...
	for (int Counter = 0; (Counter < RecommendedFilms.size() && Counter < 4); Counter++)
		RecommendationList.push_back(RecommendedFilms[Counter]);
...
void Network::SetLoggedInUser(int UserID)
{
	for (int Counter = 0; Counter < ListOfUsers.size(); Counter++)
		if(ListOfUsers[Counter]->GetUserID() == UserID)
			{
				LoggedInUser = ListOfUsers[Counter];
				return;
			}
