...
public:
	...
	std::vector<Film*> ShowDetailsOfFilm(std::string CommandLine);
	void GetParametersForShowDetail(std::string CommandLine, int& FilmID);
	std::vector<Film*> RecommendFilms(int FilmID, std::vector<int> ListOfRecommendedFilms);
