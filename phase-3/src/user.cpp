...
std::vector<int> User::GetListOfOwnedFilm(int FilmID)
{
	std::vector<int> OwnedFilms;
	for (int Counter = 0; Counter < ListOfUserFilms.size(); Counter++)
		if (ListOfUserFilms[Counter]->GetFilmID() != FilmID)
			OwnedFilms.push_back(ListOfUserFilms[Counter]->GetFilmID());
		return OwnedFilms;
}

std::vector<Film*> User::GetListPublisherFilms()
{

}

std::vector<Film*> User::GetListOfUserFilms()
{
	return ListOfUserFilms;
}