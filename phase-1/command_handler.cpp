...
#define EDIT_FILM_COMMAND "PUT films"
#define DELETE_FILM_COMMAND "DELETE films"
...
#define DELETE_COMMENT_COMMAND "DELETE comments"
#define PUT "PUT"
#define POST "POST"
#define GET "GET"
#define DELETE "DELETE"
#define FILMS_FOR_PUT "films"
...
#define FILMS_FOR_DELETE "films"
#define COMMENTS_FOR_DELETE "comments"
...
bool CommandHandler::DoesItHaveRequestError(std::string Command)
{
	...
	if (Request != PUT && Request != POST && Request != GET && Request != DELETE)
		throw BadRequest();
...
bool CommandHandler::DoesItHaveFindError(std::string Command, int LetterCounter, std::string Request)
{
	...
	{
		if (Order == FILMS_FOR_DELETE || Order == COMMENTS_FOR_DELETE)
			return false;
	}
...
void CommandHandler::RunCommand(std::string Command)
{
	...
	else if (Command.substr(0, 9) == EDIT_FILM_COMMAND)
		VirtualNetwork->EditFilmInformation(Command);
	else if (Command.substr(0, 12) == DELETE_FILM_COMMAND)
	...
	else if (Command.substr(0, 15) == DELETE_COMMENT_COMMAND)
