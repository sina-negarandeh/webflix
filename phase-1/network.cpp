...
std::string Network::Hash(std::string Input)
{
	std::string HashString = "psswrd";
	for(int Counter = 0; Counter < Input.size(); Counter++)
	{
		HashString += std::to_string(Input[Counter] - 'a' + 13);
	}
	return HashString;
}
...
std::vector<Film*> Network::SortFilmsByRate(std::vector<Film*> FilmsSortByRates)
{
	for (int FirstCounter = FilmsSortByRates.size() - 1; 0 <= FirstCounter; FirstCounter--)
	{
		int Min = FirstCounter;
		for (int SecondCounter = FirstCounter - 1; 0 <= SecondCounter; SecondCounter--)
		{
			if (FilmsSortByRates[Min]->GetRate() > FilmsSortByRates[SecondCounter]->GetRate())
			{
				Film* Temp = FilmsSortByRates[Min];
				FilmsSortByRates[Min] = FilmsSortByRates[SecondCounter];
				FilmsSortByRates[SecondCounter] = Temp;
				Min = SecondCounter;
			}
		}
	}
	return FilmsSortByRates;
}

std::vector<Film*> Network::SortFilmsByFilmID(std::vector<Film*> FilmsSortByRates)
{
	for (int FirstCounter = FilmsSortByRates.size() - 1; 0 <= FirstCounter; FirstCounter--)
	{
		int Min = FirstCounter;
		...
		{
			if (FilmsSortByRates[Min]->GetRate() == FilmsSortByRates[SecondCounter]->GetRate() &&
				FilmsSortByRates[Min]->GetFilmID() < FilmsSortByRates[SecondCounter]->GetFilmID())
			{
				Film* Temp = FilmsSortByRates[Min];
				FilmsSortByRates[Min] = FilmsSortByRates[SecondCounter];
				FilmsSortByRates[SecondCounter] = Temp;
...
std::vector<Film*> Network::SortFilms()
{
	std::vector<Film*> FilmsSortByRates = ListOfFilms;
	FilmsSortByRates = SortFilmsByRate(FilmsSortByRates);
	return SortFilmsByFilmID(FilmsSortByRates);
}

void Network::RecommendFilms(int FilmID)
{
	std::vector<Film*> FilmsSortByRates = SortFilms();
	std::vector<Film*> RecommendedFilms; 

	for (int Counter = 0; Counter < FilmsSortByRates.size(); Counter++)
		if (FilmsSortByRates[Counter]->IsAvailable() && (!LoggedInUser->DoesHaveThisFilm(FilmsSortByRates[Counter])) 
			&& (FilmsSortByRates[Counter]->GetFilmID() != FilmID))
			RecommendedFilms.push_back(FilmsSortByRates[Counter]);
...
bool Network::IsInBestRatedFilms(std::vector<Film*> FilmsSortByRates, Film* ChosenFilm)
{
	for (int Counter = 0; Counter < FilmsSortByRates.size(); Counter++)
		if (ChosenFilm == FilmsSortByRates[Counter])
			return true;
	return false;
}
